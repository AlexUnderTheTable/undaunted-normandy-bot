...
## 0. Портативный Node (без установки в систему)

Node на этой машине стоит портативно: распакованный официальный zip в
`~/.local/node`, без установщика, без записи в реестр и без прав
администратора. В системный PATH он **не прописан** — чтобы ничего не
конфликтовало и чтобы удаление сводилось к стиранию папки
(`rm -rf ~/.local/node`).

Перед `npm`-командами добавьте его в PATH текущей сессии:

```bash
export PATH="$USERPROFILE/.local/node:$PATH"   # bash
```
...
Версия: Node v24.20.0 LTS + npm 11.19.0. Обновить = скачать новый zip с
nodejs.org, распаковать поверх (или рядом) — старую папку удалить.
